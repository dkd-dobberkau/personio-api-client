...
- **`PersonioClient` (V1 authentication)** — credentials for `POST /v1/auth`
  are now sent in the JSON request body instead of the URL query string.
  Personio rejects query-string credentials with `403 Forbidden` as of
  **2026-12-01** (industry-standard practice: secrets must never appear in
  URLs, where they can leak into logs and caches). Integrations using this
  client must upgrade before that date. No public API, credential, or
  token-format changes are required — only the on-the-wire auth request.
